...
- ✅ **Быстрый режим** — интерполяция NDVI по GPS-точкам (без склейки)
- ✅ **Режим ортомозаики** — склейка через cv2.Stitcher + геореференсинг
...
**Статусы:**
- 🔄 **Обработка снимков (fast)** — интерполяция NDVI по GPS-точкам
- 🔄 **Обработка снимков (orthomosaic)** — склейка в ортомозаику
...
```json
{
  "message": "Запущена обработка снимков (orthomosaic mode).",
  "task_id": "abc123",
  "field_id": 123,
  "scan_id": 456,
...
### Пример 1: Быстрый режим, пшеница 50 га

**Данные:**
- Дрон: DJI Mavic 3M
- Высота: 100 м
- Снимков: 45
- Режим: fast (интерполяция)
...
### Пример 2: Ортомозаика, кукуруза 120 га

**Данные:**
- Дрон: Phantom 4 RTK
- Высота: 120 м
- Снимков: 180
- Режим: orthomosaic (склейка)
