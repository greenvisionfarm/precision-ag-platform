...
@pytest.mark.asyncio
async def test_bulk_kmz_export(http_server_client, auth_headers):
    """
    ...
    res1 = await client.fetch(f"{base_url}/api/field/add", method='POST', body=json.dumps(p1), headers=auth_headers)
    ...
    res2 = await client.fetch(f"{base_url}/api/field/add", method='POST', body=json.dumps(p2), headers=auth_headers)
    assert res2.code == 200

    # 3. Request Bulk Export
    export_url = f"{base_url}/api/field/export/kmz/all"
    response = await client.fetch(export_url, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_bulk_kmz_export_empty_db(http_server_client, auth_headers):
    """
    ...
    with pytest.raises(HTTPError) as e:
        await client.fetch(export_url, headers=auth_headers)
