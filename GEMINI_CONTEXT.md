...
*   **Backend:** Модульный Tornado API.
    *   `src/handlers/` — контроллеры (fields, owners, upload).
    *   `src/services/` — бизнес-логика (GIS расчеты, KMZ генерация).
...
3.  **Точность:** Расчет площадей в EPSG:3035 (LAEA Europe) через `gis_service.py`.
4.  **Экспорт DJI:** Генерация полетных заданий через `kmz_service.py`.
5.  **Безопасное тестирование:** Изоляция через `FIELD_MAPPER_ENV=test` (использует `test_fields.db`).
