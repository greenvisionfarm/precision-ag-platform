...
## Ключевой функционал
1.  **Земельный учет:** Границы полей, кадастровый учет, статус аренды/собственности, кадастровые номера. Отдельная страница деталей поля (`#field/ID`) с мини-картой и полной информацией.
...
4.  **Безопасное тестирование:** Изоляция рабочих данных от тестов.
    *   `FIELD_MAPPER_ENV=test` автоматически переключает БД на `test_fields.db`.
    *   Бэкенд-тесты (Pytest) и фронтенд-тесты (Jest) покрывают роутинг и API деталей.
