...
2.  **Точность:** Расчет площадей в EPSG:3035 (LAEA Europe) через `gis_service.py`.
3.  **Экспорт DJI:** Генерация полетных заданий через `kmz_service.py`.
4.  **Безопасное тестирование:** Изоляция через `FIELD_MAPPER_ENV=test` (использует `test_fields.db`).
